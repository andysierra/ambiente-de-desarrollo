    ...
    <div style={{display:'grid', gridTemplateColumns: 'repeat(5, 1fr)'}}>
      {
        Array.from(Array(placeholders).keys()).map(
          (placeholder, idx) => (
            <div className='skeleton' key={Date.now().toString()+idx}>
              <div className='skeleton-body d-flex flex-column justify-content-center'>
                <div className="skeleton"></div>
              </div>
              <div className='skeleton-footer d-flex flex-column justify-content-center'>
                <div className="skeleton"></div>
              </div>
            </div>
          )
