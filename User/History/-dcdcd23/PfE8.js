...
function addRemove(e) { 
  ...
  if(e.ctrlKey) {
    ...
    nChildren>1 && e.target.getAnimations().map(
      animation => animation.finished.then(res=>{
        console.log(res);
        e.target.parentElement === container && container.removeChild(e.target);
      })
    );
  }
  else {
    ...
    newBox.addEventListener('transitionend', function(e){console.log('roxxet');});
    newBox.addEventListener('animationend', function(e){console.log('ded');});
    newBox.addEventListener('animationiteration', function(e){console.log('loko');});
